...
All libraries in this repo are located within the `community` namespace.

## Event Dispatcher ##

No requirements.

	community.events.EventDispatcher;
	
	// Example
	var Dispatcher = function Dispatcher() {
		this.inheritFrom = community.events.EventDispatcher;
		this.inheritFrom();
...
Requires `community.events.EventDispatcher` and `community.utils.FunctionQueue`.
...
**NOTE**: to access the _real_ filesystem and not a sandboxed filesystem on **BlackBerry 10**, we must add the following before intializing our filesystem: `blackberry.io.sandbox = false;`.  We _must_ add the feature element `<feature id="blackberry.io" />` to the config.xml. [See documentation](https://developer.blackberry.com/html5/apis/blackberry.io.html).
...
All constants are static and exist as properties of the `FileManager` class.  Below, we just refer to the class as is but don't forget it exists within the `community.io` namespace.  Just like in an example above, I like to cache classes into variables as so: `var FileManager = community.io.FileManager`.
