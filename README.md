...
All libraries in this repo are located within the `blackberry.community` namespace.

## Event Dispatcher ##

No requirements.

	blackberry.community.events.EventDispatcher;
	
	// Example
	var Dispatcher = function Dispatcher() {
		this.inheritFrom = blackberry.community.events.EventDispatcher;
		this.inheritFrom();
...
Requires `blackberry.community.events.EventDispatcher` and `blackberry.community.utils.FunctionQueue`.
...
To Free you of this burden, I have written the `blackberry.community.io.FileManager` class.  It essentially provides a way to queue filesystem operations one after the other and handle for you the creation of non-existent directories or files you've requested.  It gives you the freedom of listening to one generic error event or events specific to certain operations (such as move, copy, etc) by inheriting the `blackberry.community.events.EventDispatcher` class.  It also allows you to create *tasks* to which you can simply listen for when they start and end when they are, eventually, completed.  The nice thing about tasks is we can group a series of operations together and not have to worry about the indiviual events; we can simply listen for the task to be finished.  The secret about all of **FileManager**'s operations is that they're not performed necessarily when you call them, but are queued until the filesystem is ready to call them by wrapping the `blackberry.community.utils.FunctionQueue` class.  Here's an example:
...
})(blackberry.community.io.FileManager);
...
All constants are static and exist as properties of the `FileManager` class.  Below, we just refer to the class as is but don't forget it exists within the `blackberry.community.io` namespace.  Just like in an example above, I like to cache classes into variables as so: `var FileManager = blackberry.community.io.FileManager`.
